...
$projects = [
    [
        'title'    => 'AIエージェントによるWordPress運用自動化',
        'category' => 'CMS構築',
        'stack'    => ['WordPress', 'PHP', 'JavaScript', 'ClaudeCode', 'GitHub'],
        'image'    => '',
        'details'  => [
            [
                'label' => '概要',
                'value' => 'セミナー告知、事例紹介、サービス紹介、資料ページ、コラムなどを掲載するWordPressサイトにおいて、ClaudeCodeを活用した運用自動化の仕組みを構築。投稿・カスタム投稿の作成や削除、既存記事の新デザインへの置き換えなどを、ClaudeCodeとのチャット指示のみで実行できるワークフローを設計した。'
            ],
            [
                'label' => '担当期間',
                'value' => '約1ヶ月'
            ...
            [
                'label' => '技術スタック詳細',
                'value' => 'WordPress / PHP / JavaScript / ClaudeCode / GitHub'
            ],
            [
                'label' => '担当内容',
                'value' => 'WordPressの投稿・カスタム投稿に対する削除、新規作成、既存記事の新デザインへの置き換え作業を、ClaudeCodeに対するチャット指示で完結できる運用フローを構築。ローカル環境での作業からステージング環境、本番環境への反映までをClaudeCodeを介して実行できる仕組みを整備した。'
            ],
            [
                'label' => '工夫した点',
                'value' => 'エンジニアではないメンバーでも運用できるよう、ClaudeCode用のRulesとSkillsを事前に整備。GitHubリポジトリから取得したサイトデータをローカル環境に配置すれば、ClaudeCodeとのチャットのみでコンテンツ更新からデプロイまで実行できる構成とした。また、AIによる作業のばらつきを防ぐためルールを明文化し、作業品質を一定に保てるよう設計した。'
            ],
            [
                'label' => '成果',
                'value' => 'WordPress運用における記事更新・ページ改修作業の効率を大幅に改善。AIエージェントを活用した運用フローを構築することで、非エンジニアでもコンテンツ更新から環境反映まで実行できる体制を整備した。'
            ],
        ],
    ],
    [
        'title'    => '面談予約システム連携 WordPressプラグイン開発',
        'category' => 'Webアプリ',
        'stack'    => ['WordPress', 'PHP', 'JavaScript', 'GraphQL', 'Next.js', 'Go (Gin)', 'MySQL', 'AWS'],
        'image'    => '',
        'details'  => [
            [
                'label' => '概要',
                'value' => 'IT教育企業在籍時に、Next.js・Go（Gin）・GraphQL・AWS・MySQLで構築された面談予約管理システムのAPIと連携するWordPressプラグインを開発。自社サービスの利用を検討するユーザーが、WordPressサイト上から面談予約を行える機能を実装した。'
            ],
            [
                'label' => '担当期間',
                'value' => '約2ヶ月'
            ...
            [
                'label' => '役割',
                'value' => 'PM / WordPressプラグイン開発 / UI調整'
            ],
            [
                'label' => '技術スタック詳細',
                'value' => 'WordPress / PHP / JavaScript / GraphQL / Next.js / Go（Gin） / MySQL / AWS'
            ],
            [
                'label' => '担当内容',
                'value' => '面談予約管理システムのAPIから面談可能日を取得し、WordPressサイト上で予約可能な日程を表示するプラグインを開発。投稿・固定ページからショートコードで呼び出せる設計とし、パラメータ指定により表示形式の変更やコンパクト表示など柔軟に利用できるよう実装した。'
            ],
            [
                'label' => '工夫した点',
                'value' => '予約可能日をGridレイアウトで構築したカレンダーUIに表示し、ユーザーが直感的に日程を選択できるインターフェースを設計。デザイナーやマーケティング担当と連携しながら、入力項目や操作フローを調整し、ユーザーが入力を負担に感じない予約導線の実装を行った。'
            ],
            [
                'label' => '成果',
                'value' => 'WordPressサイトから直接面談予約が可能となり、サービス検討ユーザーの予約導線を改善。ショートコードとパラメータ設計により複数ページで柔軟に利用できる再利用性の高いWordPressプラグインとして運用されている。'
            ],
        ],
    ],
    [
        ...
        'details'  => [
            [
                'label' => '概要',
                'value' => 'IT教育企業在籍時に、月間100万PV規模のWordPressメディアサイトにおいて、長年の運用で複雑化していた既存テーマを刷新。運用負荷の軽減とパフォーマンス改善を目的に、有名WordPressテーマへの移行を実施するとともに、インフラをAWSからレンタルサーバーへ移行した。'
            ],
            [
                'label' => '担当期間',
                'value' => '約6ヶ月'
            ...
            [
                'label' => '担当内容',
                'value' => '既存テーマから新テーマへの移行設計、WordPress実装、データ移行、AWSからレンタルサーバーへのインフラ移行、UI調整、運用改善設計を担当。約6,000件に及ぶ既存記事が新テーマでも正常に表示されるようテンプレート調整やデータ確認を行い、大量コンテンツを保持したまま安全に移行を実施した。マーケティング部門のメンバーが更新しやすい管理画面設計を行い、サイトの運用負荷軽減を目的としたCMS構造の再設計を実施した。'
            ],
            [
                'label' => '工夫した点',
            ...
            [
                'label' => '成果',
                'value' => 'サイト運用の属人化を解消し、マーケティング部門が管理画面から更新できる範囲を大幅に拡張。約6,000記事の既存コンテンツを保持したままテーマ移行を完了し、インフラコストについてもAWS運用時と比較して半分以上削減するなど、安定したメディア運用基盤を構築した。'
            ],
        ],
    ],
    [
        ...
        'details'  => [
            [
                'label' => '概要',
                'value' => '制作会社在籍時に、高級水産食材を扱う企業が運営するコーポレートサイト、飲食店舗サイト、ECサイトのリニューアルプロジェクトを担当。既存サイトの運用保守を引き継ぎつつ、新規店舗サイトのWordPress構築およびECサイトのデザインリニューアルを実施した。'
            ],
            [
                'label' => '担当期間',
                'value' => '約1年'
    ...
    [
        ...
        'image'    => '',
        'details'  => [
            ['label' => '概要',           'value' => '制作会社在籍時に、食品メーカーのコンテンツ発信基盤として、コーポレートサイト兼オウンドメディアを新規構築。ECサイトや外部ブログに分散していたレシピ記事・企業ストーリー・商品情報などを統合し、ブランドの世界観を伝える情報発信サイトとしてWordPressで構築した。'],
    ...
    [
        ...
        'details'  => [
            [
                'label' => '概要',
                'value' => '制作会社在籍時に、特定の疾患を持つ方々が集まり交流活動を行う団体の紹介サイトを新規構築。交流会・講演会・研修会などの活動内容を紹介し、会の概要や活動の雰囲気が初めて訪れた人にも分かりやすく伝わる構成でWordPressサイトを制作した。'
            ],
            [
                'label' => '担当期間',
                'value' => '約1ヶ月'
    ...
    [
        'title'    => '複数Webサイトの保守運用（30サイト以上）',
        'category' => 'Webサイト',
        'stack'    => ['WordPress', 'PHP', 'JavaScript', 'MySQL', 'Linux'],
        'image'    => '',
        'details'  => [
            [
                'label' => '概要',
                'value' => '制作会社在籍時に、企業コーポレートサイトや店舗サイトなど30サイト以上のWebサイト保守運用を担当。WordPressサイトを中心に、機能改修・不具合対応・サーバー設定・セキュリティ対応などの運用業務を継続的に実施した。'
            ],
            [
                'label' => '担当期間',
                'value' => '3年6ヶ月'
            ...
            [
                'label' => '技術スタック詳細',
                'value' => 'WordPress / PHP / JavaScript / MySQL / Linux'
            ],
            [
                'label' => '担当内容',
                'value' => 'WordPressテーマ修正、機能追加、JavaScript改修、プラグイン更新対応、セキュリティアップデート、サーバー設定変更、サイト移行作業、軽微なデザイン修正など、各サイトの継続的な運用保守を担当した。'
            ],
            [
                'label' => '工夫した点',
                'value' => '複数サイトの運用を効率化するため、更新手順の整理や作業手順の標準化を行い、定期更新や改修作業をスムーズに実施できるよう改善した。また、障害やトラブル発生時には原因調査から修正まで迅速に対応し、サイトの安定運用に貢献した。'
            ],
            [
                'label' => '成果',
                'value' => '30サイト以上のWebサイトの安定運用を継続的に支え、クライアントからの改修要望や不具合対応を迅速に実施することで、長期的なサイト運用体制の維持に貢献した。'
            ],
        ],
    ]
];
...
$careerHistory = [
    [
        'year' => '2017',
        'body' => 'ハコレコドットコム株式会社 フロントエンドエンジニア',
    ],
    [
        'year' => '2021',
        'body' => '株式会社SAMURAI フロントエンドエンジニア（現在に至る）',
    ],
];
